package com.emk.bill.yjyhtime.service.impl;
...
@Service("emkYjyhTimeService")
@Transactional
public class EmkYjyhTimeServiceImpl extends CommonServiceImpl implements EmkYjyhTimeServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(EmkYjyhTimeEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(EmkYjyhTimeEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(EmkYjyhTimeEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(EmkYjyhTimeEntity t){
		...
		map.put("yjzqyh_date", t.getYjzqyhDate());
		map.put("sjyh_date", t.getSjyhDate());
		map.put("zqyh_sate", t.getZqyhSate());
		map.put("yjwqyh_date", t.getYjwqyhDate());
		map.put("sjwqyh_date", t.getSjwqyhDate());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,EmkYjyhTimeEntity t){
 		...
 		sql  = sql.replace("#{yjzqyh_date}",String.valueOf(t.getYjzqyhDate()));
 		sql  = sql.replace("#{sjyh_date}",String.valueOf(t.getSjyhDate()));
 		sql  = sql.replace("#{zqyh_sate}",String.valueOf(t.getZqyhSate()));
 		sql  = sql.replace("#{yjwqyh_date}",String.valueOf(t.getYjwqyhDate()));
 		sql  = sql.replace("#{sjwqyh_date}",String.valueOf(t.getSjwqyhDate()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("emk_yjyh_time",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
