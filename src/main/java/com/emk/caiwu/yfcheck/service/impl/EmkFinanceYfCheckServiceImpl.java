...
public class EmkFinanceYfCheckServiceImpl extends CommonServiceImpl implements EmkFinanceYfCheckServiceI {
	...
	private void doDelBus(EmkFinanceYfCheckEntity t) throws Exception{
 	...
 	private Map<String,Object> populationMap(EmkFinanceYfCheckEntity t){
		...
		map.put("yhz_check_no", t.getYhzCheckNo());
		map.put("fkxmlb", t.getFkxmlb());
		map.put("yftz_date", t.getYftzDate());
		map.put("out_formn_no", t.getOutFormnNo());
		map.put("hdfyfk_no", t.getHdfyfkNo());
		map.put("ysfyfk_no", t.getYsfyfkNo());
		map.put("ylmldyfyfk_no", t.getYlmldyfyfkNo());
		map.put("test_no", t.getTestNo());
		map.put("zdfyfk_no", t.getZdfyfkNo());
		map.put("money", t.getMoney());
		map.put("yjfk_date", t.getYjfkDate());
	 ...
	 */
 	public String replaceVal(String sql,EmkFinanceYfCheckEntity t){
 		...
 		sql  = sql.replace("#{yhz_check_no}",String.valueOf(t.getYhzCheckNo()));
 		sql  = sql.replace("#{fkxmlb}",String.valueOf(t.getFkxmlb()));
 		sql  = sql.replace("#{yftz_date}",String.valueOf(t.getYftzDate()));
 		sql  = sql.replace("#{out_formn_no}",String.valueOf(t.getOutFormnNo()));
 		sql  = sql.replace("#{hdfyfk_no}",String.valueOf(t.getHdfyfkNo()));
 		sql  = sql.replace("#{ysfyfk_no}",String.valueOf(t.getYsfyfkNo()));
 		sql  = sql.replace("#{ylmldyfyfk_no}",String.valueOf(t.getYlmldyfyfkNo()));
 		sql  = sql.replace("#{test_no}",String.valueOf(t.getTestNo()));
 		sql  = sql.replace("#{zdfyfk_no}",String.valueOf(t.getZdfyfkNo()));
 		sql  = sql.replace("#{money}",String.valueOf(t.getMoney()));
 		sql  = sql.replace("#{yjfk_date}",String.valueOf(t.getYjfkDate()));
