...
<t:tabs id="materailTabId" iframe="false" heigth="600px" tabPosition="top" fit="true" >
    ...
    <c:if test="${ param.node eq 'cgjlshTask' ||  param.node eq 'ckyrkTask' ||  param.node eq 'cksqdTask'  ||  param.node eq 'jsylqTask' ||  param.state eq '2'}">
        <t:tab title="包装辅料入库申请单" id="imaterialFrm"  heigth="600px"  width="100%" iframe="emkMInStorageController.do?goUpdate&type=2&materialNo=${param.id}" icon="fa fa-calendar"></t:tab>
    </c:if>
    <c:if test="${param.node eq 'cksqdTask' || param.node eq 'cgjlshTask' ||  param.node eq 'jsylqTask' ||  param.state eq '2'}">
        <t:tab title="包装辅料出库申请单" id="omaterialFrm"  heigth="600px"  width="100%" iframe="emkMOutStorageController.do?goUpdate&type=2&materialNo=${param.id}" icon="fa fa-calendar"></t:tab>
    </c:if>
